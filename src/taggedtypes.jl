...
function _read_tagged_data(io::IO, sig::Signature, size, ver)
    ...
    UnknownTaggedData{sig}(read(io, size - 4))
end
...
function _read_tagged_data(io::IO, ::S, size, ver) where {S<:sigtype"data"}
    read_u32(io)
    flags = read_u32(io)
    TaggedData(flags, read(io, size - 12))
end
...
function _read_tagged_data(io::IO, ::S, size, ver) where {S<:sigtype"dtim"}
    read_u32(io) # reserved
    TaggedDateTime(read_datetime(io))
end


struct Dictionary <: AbstractTaggedData{sig"dict"}
...
function _read_tagged_data(io::IO, ::S, size, ver) where {S<:Union{sigtype"mft1",sigtype"mft2"}}
    ...
    LUT(mat, input_curves, clut, output_curves)
end
...
function _read_tagged_data(io::IO, ::S, size, ver) where {S<:Union{sigtype"mAB ",sigtype"mBA "}}
    pos = position(io) - 4
    read_u32(io) # reserved
    Ni = Int(read_u8(io))
    No = Int(read_u8(io))
    ...
    if iszero(offset_mat)
        ...
        mat = read_u16_3x3(io)
        vec = ntuple(_ -> Float64(read_u16(io)), 3)
    end
    ...
    LUT(bcurves, mat, vec, mcurves, clut, acurves)
end
...
function _read_curve(io::IO, ver)
    ...
    error("invalid curve signature: $sig")
end

struct Mesurement <: AbstractTaggedData{sig"meas"}
...
struct MultiProcessElements <: AbstractTaggedData{sig"mpet"}

end

struct NamedColor2 <: AbstractTaggedData{sig"ncl2"}

end


struct ProfileSequenceDesc <: AbstractTaggedData{sig"psec"}

end

struct ProfileSequenceIdentifier <: AbstractTaggedData{sig"psid"}
...
function MultiLocalizedUnicode(text::TaggedText)
    MultiLocalizedUnicode([MLURecord(:en, :US, text.text)])
end
function _read_tagged_data(io::IO, ::S, size, ver) where {S<:sigtype"text"}
    read_u32(io) # reserved
    TaggedText(read_ascii(io, size - 8))
end
...
struct TextDescription <: AbstractTaggedData{sig"desc"}
    ...
    function TextDescription(text, ascii="", scriptcode=0x0000, script="")
        new(text, ascii, scriptcode, script)
    end
end
...
function _check_size(actual, expected)
    actual == expected || error("size mismatch")
end
